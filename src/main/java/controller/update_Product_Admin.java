...
@MultipartConfig
@WebServlet(name = "updateProductAdminServlet",urlPatterns = "/admin/product/update/*")
public class update_Product_Admin extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String pathInfo = req.getPathInfo();
		  if (pathInfo == null || pathInfo.isEmpty()) {
	            resp.sendRedirect("/admin/product"); // Nếu không có ID, quay lại danh sách user
	            return;
	        }
		  // Cắt phần pathInfo để lấy ID người dùng
		  
		  String productId=pathInfo.substring(1); // Loại bỏ dấu '/' ở đầu
		  int productIds = Integer.parseInt(productId);// ép kiểu thành int
		  product product=productService.getProductById(productIds);
		  ...
		  req.getRequestDispatcher("/updateproduct_admin.jsp").forward(req, resp);
	}
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// Lấy tham số từ form
		  ...
		  // Gọi service xử lý
	        ...
	        boolean updateproduct = productService.updateProduct(productIds,product, imageParts, context);
	        if (!updateproduct) {
